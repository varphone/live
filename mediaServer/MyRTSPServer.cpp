...
MyRTSPServer* MyRTSPServer::createNew(UsageEnvironment& env,
                                      Port ourPort,
...
void MyRTSPServer ::lookupServerMediaSession(
  ...
  Boolean isFirstLookupInSession)
{
  ...
  if (!fileExists) {
    ...
    if (smsExists && isFirstLookupInSession) {
      // Remove the existing "ServerMediaSession" and create a new one, in case
      // the underlying file has changed in some way:
      removeServerMediaSession(sms);
      sms = NULL;
    }
                                        ...
                                        FILE* /*fid*/)
{
  ...
  if (strcmp(extension, ".aac") == 0) {
    ...
    NEW_SMS("DV Video");
    sms->addSubsession(
      DVVideoFileServerMediaSubsession::createNew(env, fileName, reuseSource));
